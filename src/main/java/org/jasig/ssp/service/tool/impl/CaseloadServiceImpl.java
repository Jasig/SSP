...
public class CaseloadServiceImpl implements CaseloadService {
        ...
        if (list.size() > 0) {
			...
			for (CaseloadBulkAddReassignment model : list) {
				...
				if (StringUtils.isNotBlank(model.getSchoolId())) {
					try {
						Person student = personService.getInternalOrExternalPersonBySchoolId(model.getSchoolId(), true); //slow, but need to add external only students
                        ...
                        if (StringUtils.isNotBlank(model.getCoachSchoolId()) || student.getCoach() != null) {
                            ...
                            if (StringUtils.isNotBlank(model.getCoachSchoolId())) {
                                try {
                                    coach = personService.getInternalOrExternalPersonBySchoolId(model.getCoachSchoolId(), true); //slow, but very rare need to add external only coach
	...
	private AuditPerson getAuditPerson(Map<String, AuditPerson> map, String schoolId) {
		...
		if (auditPerson==null) {
			try {
				final Person modifiedBy = personService.getInternalOrExternalPersonBySchoolId(schoolId, true); //slow, but external lookup ideally won't occur. Plus need to add external only in order to set person.id
