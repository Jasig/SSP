 ...
 */
@Repository
public class SapStatusDao extends AbstractReferenceAuditableCrudDao<SapStatus>
		implements AuditableCrudDao<SapStatus> {

	public SapStatusDao() {
		super(SapStatus.class);
	}
	
	public SapStatus getByCode(final String code) {
		// TODO: (performance) Perfect example of data that should be cached
		final Criteria query = createCriteria();
		query.add(Restrictions.eq("code", code));
		return (SapStatus) query.uniqueResult();
	}
	
	@Override
	public PagingWrapper<SapStatus> getAll(final SortingAndPaging sAndP) {
