 ...
 */
@Controller
@RequestMapping("/1/reference/transferGoal")
@PreAuthorize(Permission.SECURITY_REFERENCE_TEMPLATE_WRITE)
public class TransferGoalController
		extends
		AbstractAuditableReferenceController<TransferGoal, TransferGoalTO> {

	@Autowired
	protected transient TransferGoalService service;

	@Override
	protected TransferGoalService getService() {
		return service;
	}

	@Autowired
	protected transient TransferGoalTOFactory factory;

	@Override
	protected TransferGoalTOFactory getFactory() {
		return factory;
	}

	protected TransferGoalController() {
		super(TransferGoal.class, TransferGoalTO.class);
	}
	...
	public @ResponseBody
	PagedResponse<TransferGoalTO> getAll(
		...
		final PagingWrapper<TransferGoal> data = getService().getAll(
				SortingAndPaging.createForSingleSortWithPaging(
						status == null ? ObjectStatus.ALL : status, start,
						limit, sort, sortDirection, "name"));

		return new PagedResponse<TransferGoalTO>(true, data.getResults(), getFactory()
	...
	public @ResponseBody
	TransferGoalTO create(@Valid @RequestBody final TransferGoalTO obj)
	...
	public @ResponseBody
	TransferGoalTO save(@PathVariable final UUID id, @Valid @RequestBody final TransferGoalTO obj)
