...
(function($, fluid) { 
	ssp.ReportSelector = function(container, options) {
		...
		var loadAddressesForm = function(){
			...
			loadGroupInput("/ssp/api/1/reference/programStatus/", that.locate('programStatusGroup'));
			loadGroupInput("/ssp/api/1/reference/studentType/", that.locate('studentTypeGroup'));
			loadGroupInput("/ssp/api/1/reference/specialServiceGroup/", that.locate('specialServiceGroup'));
			loadGroupInput("/ssp/api/1/reference/referralSource/", that.locate('referralSourceGroup'));
		}
		...
		// Courses Select
		var reportsSelect = that.locate('reportsSelect');
		reportsSelect.append('<option value="addressesForm">Address Labels Report</option>');
		reportsSelect.append('<option value="specialServicesForm">Special Services Report</option>');
