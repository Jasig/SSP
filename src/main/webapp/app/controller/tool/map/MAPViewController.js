...
Ext.define('Ssp.controller.tool.map.MAPViewController', {
    ...
    control: {
        ...
        'showTemplateElectives':{
            selector: '#showTemplateElectives',
            listeners: {
                click: 'onShowTemplateElectives'
            }
        }
    },
    resetForm: function() {
	...
	init: function() {
        ...
        me.appEventsController.assignEvent({eventName: 'onNewTemplateRequest', callBackFunc: me.onNewTemplateRequest, scope: me});

        if (isTemplateFlag === true) {
    ...
    onShowTemplateElectives: function(button) {
        var me=this;
        ...
        if ( (me.currentMapPlan.get('isTemplate') && me.currentMapPlan.isDirty(me.semesterStores)) ) {
            ...
            me.electiveCoursesPopUp = Ext.create('Ssp.view.admin.forms.map.MapElectiveCourses', {
                currentTemplateGetId: me.currentMapPlan
            });
	...
	onSavePlanRequest: function(values){
		...
		if (me.savePlanPopUp == null || me.savePlanPopUp.isDestroyed) {
            me.savePlanPopUp = Ext.create('Ssp.view.tools.map.SavePlan', {
                hidden: true,
                saveAs: false,
                viewToClose: values.viewToClose,
                loaderDialogEventName: values.loaderDialogEventName
            });
        }
		me.savePlanPopUp.show();
    },
	...
	onSaveTemplateRequest: function(values){
		...
		if (me.saveTemplatePopUp == null || me.saveTemplatePopUp.isDestroyed) {
            me.saveTemplatePopUp = Ext.create('Ssp.view.tools.map.SaveTemplate', {
                hidden: true,
                saveAs: false,
                viewToClose: values.viewToClose,
                loaderDialogEventName: values.loaderDialogEventName
	...
	oncreateNewMapPlanButton: function(button){
    	...
    	if (me.currentMapPlan.isDirty(me.semesterStores)) {
		    ...
		    if (!isTemplateFlag) {
                ...
                me.appEventsController.getApplication().fireEvent("onBeforePlanLoad");
                me.mapEventUtils.createNewTemplate();
            }
		}
    },
	...
	onCurrentMapPlanChange: function(){
		...
		if ( me.currentMapPlan.get('isTemplate') == true || (isTemplateFlag && me.currentMapPlan.get('isTemplate') == false) ) {
            ...
            me.getShowTemplateElectives().show();
            ...
            if (me.authenticatedPerson.hasAccess('TEMPLATE_TOOL')) {
                ...
                me.getShowTemplateElectives().hide();
            }
	...
	destroy:function() {
        ...
        me.appEventsController.removeEvent({eventName: 'onNewTemplateRequest', callBackFunc: me.onNewTemplateRequest, scope: me});
        ...
        if (me.electiveCoursesPopUp !=null && !me.electiveCoursesPopUp.isDestroyed) {
            me.electiveCoursesPopUp.close();
        }
    ...
    onNewTemplateRequest: function() {
        var me = this;
        if (me.allTemplatesPopUp) {
            me.allTemplatesPopUp.destroy();
        }

        me.oncreateNewMapPlanButton();
    }
});
